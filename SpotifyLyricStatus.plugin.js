//META{"name":"DiscordLyricStatus","source":"https://raw.githubusercontent.com/DontActuallyKnow/SpotifyLyricStatus/main/SpotifyLyricStatus.plugin.js","website":"https://github.com/DontActuallyKnow/SpotifyLyricStatus"}*//

class AnimatedStatus {
	/* BD functions */
	getName() { return "Discord Lyric Status"; }
	getVersion() { return "0.0.1"; }
	getAuthor() { return "TooMuchIq"; }
	getDescription() { return "Constantly change your status to the lyrics of the song you are listening to, only works with Spotify"; }

	SetData(key, value) {
		BdApi.setData("DiscordLyricStatus", key, value);
	}

	GetData(key) {
		return BdApi.getData("DiscordLyricStatus", key);
	}
	...
	load() {
		...
		this.modules = this.modules || (() => {
			let m = []
			webpackChunkdiscord_app.push([['DiscordLyricStatus'], {}, e => { m = m.concat(Object.values(e.c)) }])
			return m
		})();
	...
	start() {
		if (this.animation.length == 0)
			BdApi.showToast("[ERROR] Discord Lyric Status: No status set!");
		else
			this.AnimationLoop();
	}
	...
	NewEditorRow({noSongText, } = {}) {
		let hbox = GUI.newHBox();
		hbox.style.marginBottom = this.kSpacing;

		let noSongTextWidget = hbox.appendChild(GUI.newInput(noSongText, "No Song playing text"));
		noSongTextWidget.style.marginRight = this.kSpacing;
